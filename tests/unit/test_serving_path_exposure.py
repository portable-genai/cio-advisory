...
``resolve_bind_host``: the Dockerfile CMD is ``uvicorn cio_advisory.api.app:app --host
0.0.0.0``, so a bound that lives only in ``main()`` never runs in a shipped process. Until
this file existed, ``CIO_PROFILE=local`` served the seeded persona list, groups and tenant
...
secret. ``CIO_S2S_TOKEN`` authenticates a calling SERVICE and no end user, so setting one is
...
The third half, and the reason a rule keyed on the profile STRING would not have been enough:
``live`` is not ``local``, and it binds the same seeded-persona adapter. A bound keyed on the
profile name would have left ``live`` serving personas to the LAN. The binding knows; the name
does not.
...
def _app_under(monkeypatch: pytest.MonkeyPatch, **env: str | None) -> Any:
    ...
    module = importlib.import_module("cio_advisory.api.app")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("local chosen, S2S token SET", {"CIO_PROFILE": "local", "CIO_S2S_TOKEN": "s3cret"}),
        ("local chosen, no token", {"CIO_PROFILE": "local"}),
        # `live` binds the seeded-persona adapter while reading as a non-local profile string.
        ("live (seeded personas), token SET", {"CIO_PROFILE": "live", "CIO_S2S_TOKEN": "s3cret"}),
        # Unset is not consent: no profile means no identity scheme was chosen at all.
        ("profile UNSET, token SET", {"CIO_S2S_TOKEN": "s3cret"}),
        # The on-premises placeholder resolves nobody until a client binds their own IdP.
        ("onprem placeholder binding", {"CIO_PROFILE": "onprem"}),
    ],
)
...
def test_the_same_posture_still_serves_a_loopback_peer(
    ...
    app = _app_under(monkeypatch, CIO_PROFILE="local")
...
def test_a_verifying_binding_stands_the_guard_down(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    app = _app_under(
        monkeypatch,
        CIO_PROFILE="gcp",
        CIO_IAP_AUDIENCE="/projects/000/global/backendServices/000",
        CIO_S2S_TOKEN="s3cret",
        # A managed process with review routing on names its console, and under gcp the IAP
        # edge audience it is reached through, or it refuses to boot.
...
def test_the_guard_is_not_derived_from_the_service_credential() -> None:
    ...
    source = Path(__file__).resolve().parents[2] / "src" / "cio_advisory" / "api" / "app.py"
