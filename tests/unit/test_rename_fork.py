...
def _diverging_args() -> Namespace:
    ...
    return Namespace(
        package="acme_advisory",
        cli="acme-advisory-cli",
        env_prefix="ACME",
        resource="acme-advisory-svc",
        dist="acme-advisory-dist",
    )
...
def test_dist_cli_and_resource_each_land_on_their_own_value() -> None:
    ...
    assert rewritten == (
        'name = "acme-advisory-dist"\n'
        'acme-advisory-cli = "acme_advisory.cli.main:app"\n'
        'resource_id = "acme-advisory-svc"\n'
        "ACME_PROFILE\n"
    )
    assert count == 4
