...
**What was observed failing first, on the live deployment, before any of this changed.**
On 2026-09-12 this service was reached through the portal's IAP edge on both hosts as the
service account the deployment's own end-to-end identity uses. Every route taking a principal
answered ``401 {"detail":"authentication required"}``. Every route taking none answered 200
through the same proxy, on the same hop, with the same token: ``/v1/personas`` returned ``[]``,
``/v1/clients/template`` returned its template, and the agent card rendered. The sibling
application ``credit-memo-drafting``, deployed earlier against an OLDER kit, answered from
inside its own application for the SAME caller through BOTH hosts. So the edge, the host, the
audience and the proxy were all ruled out by execution, and the difference was one line.

**The cause.** ``x-goog-*`` is Google's reserved namespace, and the serverless frontend REMOVES
those headers from a request entering a service so that only the platform can set them. An
embedding host behind IAP therefore cannot hand a downstream service the assertion IAP gave it
under the standard name: the host sets it, the frontend drops it, and the service refuses with
"missing IAP assertion header; request did not pass through IAP" about a request that passed
through IAP one hop earlier. The broker sends the same assertion under
``x-portal-iap-assertion`` precisely because that name is not reserved, and this adapter read
only the reserved one. It authenticated NOBODY behind the portal -- not the machine caller, and
not a human either -- while its own configuration, its audience, its reviewed tenant map and
its machine map were all correct and all unread.

**Why no test caught it.** ``test_iap_claim_half.py`` models the CLAIM half faithfully, machine
callers included, and hands the adapter its assertion under ``IAP_ASSERTION_HEADER`` -- the one
header production never delivers to an embedded application. Those tests pass and are right
about what they assert; they are silent about transport. This file is the transport half, and
the first test below is the offline reproduction of the live 401.

**The second half: the refusal was also dishonest.** ``api/security.py`` mapped every
``IdentityError`` to ``401 "authentication required"``. That sentence was false in the live
failure -- the caller had authenticated, at the edge, minutes earlier -- and it stays false for
any refusal the claim half makes AFTER the signature has been accepted: an allowlist this
deployment writes down, or a tenant it declines to map. Those callers are authenticated and
unentitled, which is a 403. A caller told "authenticate" about an entitlement goes looking for a
credential, and no credential would have helped.
"""
...
from cio_advisory.adapters.gcp import iap_identity
from cio_advisory.adapters.gcp.iap_identity import IapIdentityAdapter
from cio_advisory.domain.identity import IdentityError, RequestContext
from cio_advisory.ports.identity import AuthorizationRefusedError, EndUserAuthUnavailableError

_AUDIENCE = "/projects/1234567890/global/backendServices/42"

#: The deployment's own end-to-end identity, in the shape a real IAP assertion carries it: an
...
_MACHINE = "portal-e2e@demo-project.iam.gserviceaccount.com"
...
def test_a_machine_caller_the_map_admits_resolves_when_the_host_forwarded_the_assertion(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The offline reproduction of the live 401, and the whole of the fix.

    The assertion arrives under the FORWARDED name only, which is the only shape an embedded
    application ever sees, and the claims are a real service account's: no ``hd``, an
    ``@...iam.gserviceaccount.com`` address, the provider's ``sub``. Against the adapter as it
    shipped this refused with "missing IAP assertion header" and the reviewed machine map below
...
def test_a_human_is_refused_by_the_same_transport_defect_and_admitted_by_the_same_fix(
    ...
    A browser walkthrough of the embedded console looked healthy because the console's own
    unauthenticated routes answered, not because a human resolved. Behind the portal the
    reserved header is absent for EVERY caller, so a mapped human domain was refused exactly as
    hard as the service account was.
    """
    monkeypatch.setenv(
        iap_identity._IAP_TENANT_DOMAINS_ENV, _json.dumps({"bank.example": "reference-bank"})
    )
    monkeypatch.setenv(
        iap_identity._IAP_GROUPS_ENV, _json.dumps({"bank.example": ["group:cio-analyst"]})
    )
    ...
    principal = _resolve({iap_identity._PORTAL_ASSERTION_HEADER: _token()}, human)
...
def test_an_allowlisted_deployment_refuses_an_unnamed_machine_with_403_not_401(
    ...
    sends an operator hunting for a credential that already worked.
    """
    monkeypatch.delenv(iap_identity._IAP_MACHINE_TENANTS_ENV, raising=False)
...
def test_the_api_answers_an_authorization_refusal_with_403_and_a_reason(
    ...
    from cio_advisory.api import security
