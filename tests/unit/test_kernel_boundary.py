...
able to import the vertical-neutral kernel **without** dragging in the wealth-advisory
artifacts it is going to rewrite.

So the primary assertion is executed, not read: a fresh interpreter imports
``cio_advisory.domain.kernel`` and reports whether ``cio_advisory.domain.models`` ended up
in ``sys.modules``. Against the previous shim that subprocess printed the vertical module,
so this test was RED before the split.
"""
...
from cio_advisory.domain import kernel, models

SRC = Path(__file__).resolve().parents[2] / "src"
KERNEL_PATH = SRC / "cio_advisory" / "domain" / "kernel.py"
...
# The wealth-advisory artifacts a fork rewrites. None may live in the kernel.
VERTICAL_NAMES = (
    "AdvisoryBriefing",
    "AssetClass",
    "ClientProfile",
...
def test_importing_the_kernel_does_not_import_the_vertical_models() -> None:
    ...
    imported = _import_probe("cio_advisory.domain.kernel")
    assert "cio_advisory.domain.kernel" in imported
    assert "cio_advisory.domain.models" not in imported, (
...
def test_the_vertical_models_do_import_the_kernel() -> None:
    ...
    imported = _import_probe("cio_advisory.domain.models")
    assert "cio_advisory.domain.kernel" in imported
...
def test_kernel_source_has_no_intra_package_imports() -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            assert not (node.module or "").startswith("cio_advisory"), node.module
        elif isinstance(node, ast.Import):
            for alias in node.names:
                assert not alias.name.startswith("cio_advisory"), alias.name
