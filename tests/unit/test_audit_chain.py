...
def test_events_round_trip_and_chain_verifies() -> None:
    adapter = _adapter()
    adapter.record(_event("briefing"))
    adapter.record(_event("suitability"))
    events = adapter.read_all()
    assert [e["action"] for e in events] == ["briefing", "suitability"]
...
def test_tampering_is_detected() -> None:
    ...
    conn.execute(
        "UPDATE audit_log SET event_json = replace(event_json, 'briefing', 'x') WHERE seq = 1"
    )
    conn.commit()
