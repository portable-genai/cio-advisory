...
from cio_advisory.adapters.platform import _s2s
from cio_advisory.adapters.platform.review_router import PlatformReviewRouter
...
from cio_advisory.domain.models import AdvisoryBriefing
...
BRIEFING = AdvisoryBriefing(client_id="client-000042")
...
def test_the_router_sends_a_freshly_minted_bearer_per_submission(
    ...
    router.route(BRIEFING, maker="analyst@bank.test", tenant="t1")
    router.route(BRIEFING, maker="analyst@bank.test", tenant="t1")
...
def test_the_minted_bearer_wins_over_a_static_token(
    ...
    PlatformReviewRouter(Settings(profile="gcp")).route(BRIEFING, maker="a@bank.test")
...
def test_without_an_audience_the_router_keeps_the_static_token(
    ...
    monkeypatch.setenv("S2S_TOKEN", "static-token")
    PlatformReviewRouter(Settings(profile="platform")).route(BRIEFING, maker="a@bank.test")
...
def test_platform_still_needs_only_the_console_url(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CIO_PROFILE", "platform")
    monkeypatch.setenv(HUMAN_REVIEW_URL_ENV, "https://review.example.test")
    assert Settings.load(CONFIG).controls.review_routing is True
