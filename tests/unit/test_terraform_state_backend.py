...
* **the state lived on one laptop.** ``providers.tf`` declared no backend, so an apply wrote
  ``terraform.tfstate`` beside the code: the only record of the BigQuery dataset, its tables
...
* **a partial backend must not reach the offline proof.** ``make tf-check`` initialises with
...
* **a documented init that names no backend is an instruction to start from empty state.** An
  empty prefix plans the dataset and the key ring as new, and both creates fail because both
...
def test_the_offline_terraform_proof_never_initialises_the_backend() -> None:
    """The offline target initialises with the backend OFF, however it spells the command.

    This repository's target runs ``terraform -chdir=<dir> init``, the sibling stacks ``cd``
    first, and the assertion is about the property rather than the spelling: the init the gate
    runs must carry ``-backend=false``, because an init that configured the partial backend
    would need a bucket and credentials, which the gate contract forbids.
    """
    recipe = _recipe("tf-check")
...
def test_the_deploy_docs_migrate_the_existing_state_rather_than_recreate_it() -> None:
    for source in (_RUNBOOK, _TF_README):
        ...
        assert migrations, (
            ...
            "the BigQuery dataset that already exists fails"
        )
