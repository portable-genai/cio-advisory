...
exactly as ``CIO_PROFILE=gcp`` constructs it. The only thing faked is the HTTP fetch of
...
``CIO_IAP_MACHINE_TENANTS_JSON``, never by its domain, which every account in a project
...
its ``iap_matrix_path``, sets ``CIO_REQUIRE_IAP_MATRIX=1`` (the prefix comes from the
``CIO_PROFILE=`` line in ``.env.example``) and fails if the run reports a skip. Under that
...
from cio_advisory.adapters.gcp.iap_identity import (
...
from cio_advisory.ports.identity import (
...
_REQUIRE_ENV = "CIO_REQUIRE_IAP_MATRIX"
...
AUDIENCE_ENV = "CIO_IAP_AUDIENCE"
TENANT_DOMAINS_ENV = "CIO_IAP_TENANT_DOMAINS_JSON"
GROUPS_ENV = "CIO_IAP_GROUPS_JSON"
MACHINE_TENANTS_ENV = "CIO_IAP_MACHINE_TENANTS_JSON"
...
def test_the_reviewed_group_map_grants_a_verified_human_its_roles(
    ...
    monkeypatch.setenv(GROUPS_ENV, json.dumps({"bank.example": ["group:cio-analyst"]}))
    principal = _resolve(_assertion(keys), header)
    assert principal.principals == (f"user:{HUMAN}", "group:cio-analyst")
