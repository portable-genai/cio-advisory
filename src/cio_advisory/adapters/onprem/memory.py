"""On-prem placeholder for ``MemoryPort`` : the on-premise migration target.

One of the reversibility (P-02) migration placeholders: in the managed profile this port
binds to the Agent Platform Memory Bank adapter; switching ``profile`` to ``onprem``
rebinds it here. The adapter constructs cleanly with **no external dependencies** and
structurally satisfies the same Protocol as the managed adapter. Porting durable RM
memory to an on-premise store is *only* a matter of filling these bodies in : the core
...
class OnPremMemoryAdapter:
    """Placeholder memory adapter for the on-prem profile."""
