"""On-prem placeholder for ``SessionPort`` : the on-premise migration target.

One of the reversibility (P-02) migration placeholders: in the managed profile this port
binds to the Agent Platform Sessions adapter; switching ``profile`` to ``onprem`` rebinds
it here. The adapter constructs cleanly with **no external dependencies** and structurally
satisfies the same Protocol as the managed adapter. Porting per-client session state to an
on-premise store is *only* a matter of filling these bodies in : the core domain logic is
unchanged.
"""
...
class OnPremSessionAdapter:
    """Placeholder session adapter for the on-prem profile."""
