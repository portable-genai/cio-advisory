...
binds to the A2A registry adapter; switching ``profile`` to ``onprem`` rebinds it here.
The adapter constructs cleanly with **no external dependencies** and structurally
satisfies the same Protocol as the managed adapter. Porting agent registration to an
on-premise catalog is *only* a matter of filling these bodies in : the core domain logic
is unchanged.
"""
