"""On-prem placeholder for ``GuardrailPort`` : the on-premise migration target.

One of the reversibility (P-02) migration placeholders: in the managed profile this port
binds to the Model Armor adapter; switching ``profile`` to ``onprem`` rebinds it here.
The adapter constructs cleanly with **no external dependencies** and structurally
satisfies the same Protocol as the managed adapter. ``screen`` deliberately raises rather
than fail-opening: an unimplemented guardrail must never silently allow traffic, so
porting on-premise *must* provide a real screening backend. Filling this body in is the
