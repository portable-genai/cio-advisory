...
# local -> adapters -> cio_advisory -> src -> <repo>.
...
class LocalOfflineEvalAdapter:
    ...
    def gate(self, target: str) -> bool:
        """Refuse: the offline profile holds no promotion authority.

        ``evaluate`` genuinely works here : it is the deterministic scorer the merge gate runs
        ...
        raise NotImplementedError(
            ...
            "model-quality-gate decision and needs CIO_PROFILE=platform or gcp. "
            "Run `python eval/run_eval.py --mode smoke` for the offline pre-merge check."
        )
    ...
    def _load_run_eval():  # type: ignore[no-untyped-def]
        if not _EVAL_SCRIPT.exists():
            return None
        spec = importlib.util.spec_from_file_location("cio_local_run_eval", _EVAL_SCRIPT)
        if spec is None or spec.loader is None:
            return None
        module = importlib.util.module_from_spec(spec)
        sys.modules.setdefault("cio_local_run_eval", module)
