...
laptop run has no public-web grounding backend, so ``enabled`` is ``False`` and ``ground``
returns no web citations rather than raising, exactly like a sealed perimeter. The
advisory pipeline simply skips the grounding step. SDK-free and unconditional (there is no
emulator for web grounding).
"""
