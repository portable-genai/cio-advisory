...
The ``live`` profile's generator. Briefing narratives and talking points
are produced by a model running on the operator's own machine; the client portfolio
data in those prompts never leaves it. Suitability verdicts stay with the
deterministic policy engine: this model only narrates.
...
rather than crashing the briefing).
...
class GemmaLocalLLMAdapter:
    ...
    def classify(self, text: str, labels: list[str]) -> str:
        ...
        try:
            ...
            # Triage only routes; a server blip must not fail a briefing.
            return labels[0]
