...
FinOps) instead of B3 calling Cloud Logging directly. This adapter implements
...
Client PII is removed at the domain boundary (P-04) before the event reaches this adapter,
so the JSON body it sends is already safe to persist. The base URL is read from
...
class RemoteAuditError(AdvisoryError):
