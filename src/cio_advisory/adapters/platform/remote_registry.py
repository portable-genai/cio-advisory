...
In the full platform deployment, B3's A2A AgentCard is published to and resolved through
the shared ``agent-registry`` service (A2A + MCP catalog) rather than served only from
B3's own ``/.well-known/agent-card.json``. This adapter implements
...
class RemoteRegistryError(AdvisoryError):
...
class RemoteRegistryAdapter:
    ...
    def _parse_card(body: dict) -> AgentCard:
        ...
        return AgentCard(
            ...
            skills=skills,
            provider=str(body.get("provider", "cio-advisory")),
        )
