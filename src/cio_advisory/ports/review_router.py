"""ReviewRouterPort: the boundary that routes an escalated advisory briefing to Hrz7 (rule R8).
...
item to the Hrz7 Human-Review & Maker-Checker Console rather than terminate the escalation in a
...
class ReviewRouterPort(Protocol):
    def route(self, briefing: AdvisoryBriefing, *, maker: str, tenant: str = "") -> None:
        """Route an escalated briefing to Hrz7 for review (idempotent per client is ideal)."""
        ...
