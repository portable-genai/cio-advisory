...
deployed conversational agent, **Agent Platform Sessions** for per-client state, and
**Agent Platform Memory Bank** for durable RM preferences/facts.
"""
...
from ..domain.models import AdvisoryBriefing, LlmMessage, MemoryItem, Session


@runtime_checkable
class AgentRuntimePort(Protocol):
    def query(self, session: Session, message: str) -> AdvisoryBriefing:
        """Send an RM message to the hosted agent within a session and get a briefing."""
        ...
