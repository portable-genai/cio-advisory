...
# reader of CIO_PROFILE that the profile drift guard permits.
...
app.add_middleware(
    ...
    # The console calls this service CROSS-ORIGIN (``NEXT_PUBLIC_API_BASE``), and a browser
    # hides every response header a cross-origin script was not told it may read. Unlisted,
    # the two answer-provenance headers below would reach the browser and never the model
    # pills, which would sit on the configured model forever with every test green.
...
# headers, so what a pill names is what answered, never what configuration says would. A request
# that noted nothing (health, the portfolio view, a briefing served entirely from cache) sends
