...
@dataclass(frozen=True)
class LiveSettings:
    """The ``live`` profile: real grounded market research + a local model server.

    Narrative generation runs on any OpenAI-compatible ``/chat/completions`` endpoint
    on the operator's machine. The house-view themes come from Gemini google_search
    grounded research over real published market commentary (requires
    GOOGLE_CLOUD_PROJECT + ADC, like Doc1's live research), cached on disk so a demo
    re-run does not re-research the same day.
    """
