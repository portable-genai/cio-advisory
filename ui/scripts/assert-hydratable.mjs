...
//
//   1. The response carries a `script-src` with a nonce.
//   2. EVERY `<script>` tag in the document carries that same nonce.
//   3. The rest of the standard directives are present, and `frame-ancestors` is not empty.
//
// Rule 2 is the one that matters, and it is the one no header assertion can express: the header
...
try {
  ...
  if (exited) {
    ...
    for (const [name, value] of directives) {
      if (!value) fail(`the CSP directive \`${name}\` is empty, which browsers discard as a parse error`);
    }
