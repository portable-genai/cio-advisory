...
const STANDALONE = "http://api.test:8091";
const PORTAL = "/apps/cio-advisory/api";
...
test("only this console's own API base counts, standalone or under the portal", () => {
  assert.equal(isConsoleApi(STANDALONE + "/v1/briefing", HERE, STANDALONE), true);
  assert.equal(isConsoleApi(new URL(STANDALONE + "/healthz"), HERE, STANDALONE + "/"), true);
  assert.equal(isConsoleApi({ url: STANDALONE + "/v1/briefing" }, HERE, STANDALONE), true);
  assert.equal(isConsoleApi("http://elsewhere.test:8091/v1/briefing", HERE, STANDALONE), false);
  assert.equal(isConsoleApi("/apps/cio-advisory/api/v1/briefing", HERE, PORTAL), true);
  assert.equal(isConsoleApi("/apps/cio-advisory/apisomething", HERE, PORTAL), false);
...
test("the wrapper reports an answer from the console's API and ignores other origins", async () => {
  ...
  await window.fetch(STANDALONE + "/v1/briefing", { method: "POST" });
  await window.fetch("http://elsewhere.test/v1/briefing");
  stop();
...
test("the wrapper is installed once however many listeners, and restored by the last", async () => {
  ...
  await window.fetch("/apps/cio-advisory/api/v1/briefing");
  stopFirst();
