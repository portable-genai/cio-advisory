 ...
 *
 * Not cosmetic. This assistant is demonstrated on a laptop and on the deployment, sometimes
 * in the same hour, and a screenshot of one is indistinguishable from the other. A viewer
 * who cannot tell which they are looking at cannot tell whether a talking point was
 * generated by Gemini or by a deterministic offline stub, and that is exactly the
 * confusion an audit-first pitch cannot afford. The page says it, always, instead of the
 * presenter saying it sometimes.
 *
...
export function ProvenanceBanner() {
  ...
  useEffect(() => {
    ...
    healthz(controller.signal)
      .then(({ raw }) => {
        if (!live || !raw) return;
        setOrigin(provenance(raw.runtime, raw.generator_model));
      })
      .catch(() => undefined);
    return () => {
      live = false;
