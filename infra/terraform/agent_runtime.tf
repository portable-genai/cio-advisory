...
#         identity, CMEK, and a data reference so the rest of the infra (IAM, perimeter, KMS)
#         is in place before the SDK deploy runs.
#   P-09 (CMEK explicit): the aiplatform service-agent key binding lives in kms.tf.
#
# WHY NO google_*_reasoning_engine RESOURCE: as of the pinned provider line there is no
# first-class Terraform resource for an Agent Runtime / reasoningEngine instance; it is
# created out-of-band by the ADK deploy step. We provision its runtime service account and a
# data reference, and document the deploy command.

# Runtime service account the deployed reasoningEngine runs as.
resource "google_service_account" "agent_runtime" {
  account_id   = "cio-advisory-runtime"
  display_name = "B3 Agent Runtime (reasoningEngine) identity"
  project      = var.project_id

  depends_on = [google_project_service.required]
}

# Least-privilege runtime bindings: call Vertex/Agent Platform, read the house-view store,
...
# Placeholder reference for the engine the SDK deploys out-of-band. Populate
# CIO_AGENT_ENGINE (settings.yaml agent_engine.resource_name) with the resource id printed
# by the deploy step. Deploy command (run after apply):
#
