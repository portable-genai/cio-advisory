...
#
# This stack deploys neither the API nor the console: an embedding host runs them from
# digest-pinned images. It still owns the repository those digests live in, because the repository
# is where residency, CMEK and tag immutability are decided, and those are this stack's concerns
...
resource "google_artifact_registry_repository" "images" {
  project       = var.project_id
  location      = var.region
  repository_id = "cio-advisory"
  description   = "Promoted cio-advisory API and console images, CMEK-encrypted."
  format        = "DOCKER"
  ...
  # deployment pins images by digest, so a policy that deleted an untagged or older version would
  # be the only resource in this stack able to delete a RUNNING deployment's image, and it would
  # do it on a timer rather than in a plan anybody read.
...
locals {
  ...
  # service-account emails is refused outright; the VALUE may be computed freely.
...
output "image_registry" {
  description = "Registry path the deployment promotes this application's images into."
