...
Rewrites the package name, the CLI entry point, the ``CIO_`` env-var prefix, and the
...
changed), then run the gate. See docs/ADOPTING.md for the full checklist (fixtures, PII
pack, suitability policy, IdP wiring, region) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "cio_advisory"
_OLD_CLI = "cio-advisory"
_OLD_ENV_PREFIX = "CIO_"
_OLD_RESOURCE = "cio-advisory"
_OLD_DIST = "cio-advisory"
...
def _replacements(args: argparse.Namespace) -> list[tuple[str, str]]:
    ...
    # longer, more specific strings first. In this repo the CLI name and the resource stem
    # are the same string ("cio-advisory"); the resource replacement runs first and consumes
    # every occurrence, so pass matching --cli / --resource values for a clean rename.
    return [
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``CIO_`` when it prefixes an UPPER token
        # (CIO_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument("--resource", required=True, help="new resource id stem, e.g. acme-advisory")
    ...
    print(
        "Then finish the human steps in docs/ADOPTING.md (fixtures, PII pack, policy, IdP, region)."
    )
    return 0
