...
GOLDEN = _REPO_ROOT / "eval" / "datasets" / "golden_clients.jsonl"
QUERIES = _REPO_ROOT / "eval" / "datasets" / "retrieval_queries.jsonl"
...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden clients.", ""]
    return lines
...
def _exercised_block() -> list[str]:
    ...
    verdicts = sum(len(e.expected_verdicts) for e in examples)
    gaps = sum(len(e.expected_addressed_gaps) for e in examples)
    labelled = sum(len(set(row["relevant"])) for row in queries)
    return [
        BLOCKS[1],
        "",
        f"- **{len(examples)} golden clients** in `eval/datasets/golden_clients.jsonl`, carrying",
        f"  {verdicts} expected suitability verdicts and {gaps} expected allocation gaps. The",
        ...
        f"- **{len(queries)} labelled retrieval queries** in",
        f"  `eval/datasets/retrieval_queries.jsonl`, carrying {labelled} reviewer-labelled house",
        ...
        "  it returned.",
        f"- **{len(narrative)} judged talking points** in",
