...
| **P-09** CMEK does not cascade | Met | One regional key in `infra/terraform/kms.tf` with an explicit binding per data-bearing service (BigQuery, Agent Search, Agent Runtime, Logging); `org_policy.tf` `restrictNonCmekServices` backstop where this stack owns the project's policies. |
