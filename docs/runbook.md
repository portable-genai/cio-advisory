...
| `401 {"detail":"authentication required"}` on every route taking a principal, while routes taking none answer 200 through the same proxy | The assertion is not arriving under a name this service reads. Behind an embedding host the serverless frontend strips the reserved `x-goog-*` namespace, so the host's copy of `x-goog-iap-jwt-assertion` never reaches the container and the forwarded `x-portal-iap-assertion` is the only name left | This service reads both (`docs/embedding-and-identity.md` 6a). If the symptom persists, the deployed IMAGE predates that fix: check the running revision's image digest before changing any configuration, because the audience and the reviewed maps are not the cause and editing them will not help. |
| `403` naming the caller, not `401` | The caller authenticated and this deployment admits them nothing: an allowlist that does not name them, or a tenant the reviewed maps decline to resolve | Add the exact address to `CIO_IAP_MACHINE_TENANTS_JSON`, or the sign-in domain to `CIO_IAP_TENANT_DOMAINS_JSON`. No credential change helps; see `docs/embedding-and-identity.md` 6b. |
