...
- **Demo C - REAL research and YOUR portfolio under the `live` profile** (the
  ...
  the deterministic policy engine; the narrative is generated by the Gemini API, like
...
```bash
# 1. Install the research extra: pip install -e '.[live]'
#    No local model server: every model call in this profile is the Gemini API, because
#    the house views are researched from the open web and cannot be produced without it.

# 2. Serve live (research AND generation need a GCP project + application-default creds).
